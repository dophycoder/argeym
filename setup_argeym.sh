...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _play() {
    ...
    Navigator.of(context).push(
      MaterialPageRoute<void>(builder: (_) => LobbyScreen(callsign: n)),
    );
  }
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        _saved ? 'Saved — you can change it anytime' : 'Tap save to store your callsign',
  ...
  Widget _modeSelector() {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          const Text('HIT MODE — applies to both players',
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Text(
              ...
              style: TextStyle(
                  fontSize: 12, fontWeight: FontWeight.bold, color: ready ? C.accent : C.muted),
            ),
          ),
        ],
      ),
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        style: ElevatedButton.styleFrom(
                            backgroundColor: C.panel2, foregroundColor: Colors.white),
...
class _GameplayScreenState extends State<GameplayScreen> {
  ...
  void initState() {
    ...
    _bind();
    _initCamera();
    _renderTimer = Timer.periodic(const Duration(milliseconds: 120), (_) {
      if (mounted) setState(() {});
    });
  }
  ...
  void _bind() {
    ...
    widget.socket.on('countdown', (dynamic data) {
      if (!mounted) return;
      final int c =
          (data is Map && data['count'] != null) ? (data['count'] as num).toInt() : 0;
      setState(() {
        _phase = GamePhase.countdown;
        _countdownLabel = c > 0 ? c.toString() : 'GO!';
      });
    });
    ...
    widget.socket.on('hit_taken', (dynamic data) {
      if (!mounted) return;
      final int hp =
          (data is Map && data['hp'] != null) ? (data['hp'] as num).toInt() : _hp;
      setState(() => _hp = hp);
      _flashRed();
    });
    ...
    widget.socket.on('respawn', (dynamic data) {
      if (!mounted) return;
      final int hp =
          (data is Map && data['hp'] != null) ? (data['hp'] as num).toInt() : 100;
      setState(() {
        _hp = hp;
        _alive = true;
        _ammo = _maxAmmo;
    ...
    widget.socket.on('game_ended', (dynamic data) {
      if (!mounted) return;
      final String reason = (data is Map && data['reason'] != null)
      ...
      builder: (BuildContext ctx) {
        Widget tile(String m, String title, String sub, IconData icon) {
          ...
          return ListTile(
            ...
            title: Text(title,
                style: TextStyle(
                    fontWeight: FontWeight.bold, color: sel ? modeColor(m) : Colors.white)),
          ...
          child: Column(
            ...
            children: <Widget>[
              const Padding(
                ...
                child: Text('HIT MODE',
                    style: TextStyle(
                        letterSpacing: 3, fontWeight: FontWeight.w900, fontSize: 16)),
              ),
              tile(HitMode.normal, 'NORMAL', '80 m / 12° — real aiming', Icons.verified),
              tile(HitMode.test, 'TEST', '300 m / 45° — long range, through windows',
                  Icons.science),
  ...
  Widget _buildModeTag() {
    return Positioned(
      ...
      child: Center(
        child: Text(
          modeLabel(_mode, _range, _angle).toUpperCase(),
          style: TextStyle(
              fontSize: 11,
              color: modeColor(_mode),
              letterSpacing: 2,
              fontWeight: FontWeight.bold),
        ),
      ),
  ...
  Widget _buildOpponentLayer(double w, double h) {
    final bool haveBoth = _oppLat != null &&
        _oppLng != null &&
        _sensors.lat != null &&
        _sensors.lng != null;
    if (!haveBoth) {
    ...
    final double b =
        bearingBetween(_sensors.lat!, _sensors.lng!, _oppLat!, _oppLng!);
    final double dist =
        distanceMeters(_sensors.lat!, _sensors.lng!, _oppLat!, _oppLng!);
    ...
    return Column(
      ...
      children: <Widget>[
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
          decoration: BoxDecoration(
              color: C.danger.withOpacity(0.85), borderRadius: BorderRadius.circular(8)),
          child: Text(_oppName,
              style: const TextStyle(
        ...
        Container(
          ...
          child: Text(prettyDistance(dist),
              style: const TextStyle(
                  fontSize: 13, fontWeight: FontWeight.bold, color: C.accent)),
        ),
      ],
    );
  }
  ...
  Widget _edgeArrow(bool left, double dist) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[
        Icon(left ? Icons.arrow_back_ios_new : Icons.arrow_forward_ios,
            color: C.warn, size: 40),
        const SizedBox(height: 6),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
          decoration:
          ...
          children: <Widget>[
            ...
            Align(
              ...
              child: Row(
                ...
                children: <Widget>[
                  _actionButton(
                      _reloading ? 'RELOADING' : 'RELOAD', C.panel2, _reload, Icons.autorenew),
    ...
    return Container(
      ...
      child: Center(
        child: Text(_countdownLabel,
            style: const TextStyle(
                fontSize: 88, fontWeight: FontWeight.w900, color: Colors.white)),
      ),
    );
  }
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const Text('RESPAWNING…',
                style: TextStyle(fontSize: 16, color: C.warn, letterSpacing: 2)),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: <Widget>[
            const Text('GAME OVER',
                style: TextStyle(
                    fontSize: 48, fontWeight: FontWeight.w900, letterSpacing: 4)),
            const SizedBox(height: 8),
            Text(_endReason, style: const TextStyle(fontSize: 16, color: C.muted)),
            const SizedBox(height: 16),
            Text('FRAGS  $_myKills   DEATHS  $_myDeaths',
                style: const TextStyle(
                    fontSize: 18, color: C.accent, fontWeight: FontWeight.bold)),
            const SizedBox(height: 28),
            ElevatedButton(
              onPressed: () =>
...
jobs:
  ...
  build-android:
    ...
    steps:
      ...
      # ---------------------------------------------------------------
      # FIX: geolocator_android (and some other plugins) reference
      # `flutter.compileSdkVersion` which is only available inside the
      # Flutter plugin build-script scope, not in a root subprojects {}
      # block.  The reliable fix is to set compileSdkVersion explicitly
      # in every plugin's android {} block via a Gradle init script
      # that is injected into GRADLE_USER_HOME before the build runs.
      # ---------------------------------------------------------------
      - name: Inject Gradle init script to fix plugin compileSdk
        run: |
          mkdir -p "$HOME/.gradle/init.d"
          cat > "$HOME/.gradle/init.d/fix_compile_sdk.gradle" << 'GRADLE_EOF'
          allprojects {
            afterEvaluate { project ->
              // Only touch Android library sub-projects (plugins)
...
git checkout -B main
git add -A
git commit -m "ARGEYM: fix Android plugin compileSdk via Gradle init script"
